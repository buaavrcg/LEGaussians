...
class ViTExtractor:
    ...
    def preprocess_PIL(self, pil_image: Image.Image, load_size: Union[int, Tuple[int, int]] = None) -> Tuple[torch.Tensor, Image.Image]:
        if load_size is not None:
            pil_image = transforms.Resize(load_size, interpolation=transforms.InterpolationMode.LANCZOS)(pil_image)
        prep = transforms.Compose([
            transforms.ToTensor(),
    ...
    def extract_descriptors(self, batch: torch.Tensor, layer: int = 11, facet: str = 'key',
                            bin: bool = False, include_cls: bool = False, unflatten: bool = False) -> torch.Tensor:
        """
    ...
    def reshape_descriptors(self, descriptor: torch.Tensor, bin: bool = False) -> torch.Tensor:
        if bin:
...
def plot_pac(descriptors, path):
    import matplotlib.pyplot as plt
    import numpy as np
    from sklearn.decomposition import PCA

    features = descriptors.cpu().squeeze()
    ...
    plt.imshow(pca_features.reshape(extractor.num_patches[0], extractor.num_patches[1], 3).astype(np.uint8))
    plt.savefig(f'{path}/features_extractor.png')
...
if __name__ == "__main__":
    ...
    # parser.add_argument('--load_size', default=(224, 336), type=Tuple, help='load size of the input image.')
